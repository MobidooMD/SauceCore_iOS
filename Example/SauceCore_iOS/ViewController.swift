...
class ViewController: UIViewController, UITextFieldDelegate {
    ...
    private func setupCheckBoxes() {
        let messageHandlerNames: [MessageHandlerName] = [.customCoupon, .issueCoupon, .enter, .moveExit, .moveLogin, .moveProduct, .moveBanner, .onShare, .pictureInPicture, .tokenError, .pictureInPictureOn, .sauceflexOSPictureInPicture]
    ...
    @objc private func toggleCheckBox(_ sender: UIButton) {
        ...
        if selectedMessageHandlers.contains(selectedName) {
            selectedMessageHandlers.removeAll(where: { $0 == selectedName })
            ...
            selectedMessageHandlers.append(selectedName)
    ...
    @objc private func openWebViewController() {
        ...
        if selectedMessageHandlers.contains(.sauceflexOSPictureInPicture) {
            webViewController.pipMode = false
        } else {
